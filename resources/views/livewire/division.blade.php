<div class="space-y-6">
    ...
    <x-page-header
        ...
        subtitle="Manage division information">

        <x-slot name="actions">
            <button
                type="button"
                @click="$dispatch('open-division-modal')"
    ...
    <x-data-card title="Kim-Fay Divisions" subtitle="All divisions in the organization.">
            <x-slot name="actions">
                {{-- Search + filter --}}
            ...
            <div class="lg:hidden divide-y divide-gray-100">
                        ...
                        <div class="flex items-start justify-between gap-3 mb-3">
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs mt-1">
                                    ...
                                    <span class="text-gray-500">{{ $division->department->name ?? 'N/A' }}</span>
                                </p>
                            </div>
                        </div>  
                    </div>   
                @empty
                    <div class="py-16 text-center">
                        <p class="font-medium text-gray-700">No divisions found</p>
                        <p class="text-xs text-gray-500 mt-1">Try adjusting your search or add a new division</p>
                    </div>
                @endforelse
            </div>
...
{{-- Division Creation Modal --}}
    <div
        x-data="{ isOpen: false }"
        x-on:open-division-modal.window="isOpen = true"
        x-on:close-division-modal.window="isOpen = false"
    ...
    >
        ...
        >
            ...
            <div class="flex items-start justify-between p-6 border-b border-gray-100">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">Create New Division</h3>
                    <p class="text-sm text-gray-500 mt-0.5">Add a new division to the system</p>
                </div>
            ...
            {{-- Form --}}
            <form wire:submit.prevent="createDivision" class="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
                {{-- Division Name --}}
                <div>
                ...
                <div class="relative">
                            ...
                            <div wire:loading.remove wire:target="searchHead">
                                @if(count($headResults))
                                    <div class="py-1">
                                        <div class="px-3 py-1.5 text-[10px] font-medium text-gray-400 uppercase tracking-wider border-b border-gray-100">
                                            {{ count($headResults) }} department{{ count($headResults) > 1 ? 's' : '' }} found
                                        </div>
                                        
                                        @foreach($headResults as $department)
                                            <button
                                                type="button"
                                                wire:click="selectHead({{ $department->id }})"
                                ...
                                @elseif(strlen($searchHead) >= 2)
                                    <div class="p-6 text-center">
                                        ...
                                        <p class="text-sm text-gray-500">No department found</p>
                    ...
                    @if($selectedDepartment)
                        <div class="mt-3 rounded-lg bg-green-50 border border-green-200 p-3 flex items-center gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="text-xs text-green-600 font-medium">Selected Department</div>
                ...
                <div class="flex justify-end gap-2 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit"
                        ...
                        wire:loading.attr="disabled"
                    >
                        <span wire:loading.remove wire:target="createDivision">Add Division</span>
                        <span wire:loading wire:target="createDivision" class="flex items-center gap-2">

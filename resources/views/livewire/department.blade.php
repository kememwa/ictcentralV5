<div class="space-y-6">
    ...
    <x-page-header
        ...
        <x-slot name="actions">
            <button
                type="button"
                @click="$dispatch('open-department-modal')"
    ...
    <x-data-card title="Kim-Fay Departments" subtitle="All departments in the organization.">
            <x-slot name="actions">
                {{-- Search + filter --}}
                ...
                <table class="w-full text-xs mb-3">
                    <thead class="bg-gray-50 text-left uppercase tracking-wider text-[11px] text-gray-500">
                        <tr>
                            ...
                            <th class="px-3 py-2">HOD Name</th>
                        </tr>
                    </thead>
            ...
            <div class="lg:hidden divide-y divide-gray-100">
                        ...
                        <div class="flex items-start justify-between gap-3 mb-3">
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs mt-1">
                                    <span class="font-semibold text-gray-600">HOD:</span>
                                    <span class="text-gray-500">{{ $department->hod->name ?? 'N/A' }}</span>
                                </p>
                            </div>
                        </div>  
                    </div>   
                @empty
                    <div class="py-16 text-center">
                        <p class="font-medium text-gray-700">No departments found</p>
                        <p class="text-xs text-gray-500 mt-1">Try adjusting your search or add a new department</p>
                    </div>
                @endforelse
            </div>
...
{{-- Department Creation Modal --}}
    <div
        x-data="{ isOpen: false }"
        x-on:open-department-modal.window="isOpen = true"
        x-on:close-department-modal.window="isOpen = false"
        ...
        >
            ...
            <div class="flex items-start justify-between p-6 border-b border-gray-100">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-0.5">Add a new department to the system</p>
                </div>
            ...
            {{-- Form --}}
            <form wire:submit.prevent="createDepartment" class="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
                {{-- Department Name --}}
                <div>
                ...
                <div class="relative">
                                ...
                                @if(count($headResults))
                                    <div class="py-1">
                                        <div class="px-3 py-1.5 text-[10px] font-medium text-gray-400 uppercase tracking-wider border-b border-gray-100">
                                            {{ count($headResults) }} employee{{ count($headResults) > 1 ? 's' : '' }} found
                                        </div>
                                        
                                        @foreach($headResults as $user)
                                            <button
                                                type="button"
                    ...
                    @if($department_head)
                        <div class="mt-3 rounded-lg bg-green-50 border border-green-200 p-3 flex items-center gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="text-xs text-green-600 font-medium">Selected Head of Department</div>
                ...
                <div class="flex justify-end gap-2 pt-4 border-t border-gray-100">
                    ...
                    >
                        ...
                        <span wire:loading wire:target="createDepartment" class="flex items-center gap-2">
